...
// AI 구조화 출력(output_config.format / response_format)에 사용하는 스키마.
// src/lib/types.ts 의 AIReplyResult / AIReplyItem 과 구조를 반드시 맞춘다.
// STEP 8: 답변 유형을 고정된 enum이 아니라 상황에 맞는 자유 라벨로 바꿨다(배열 순서 = 추천 우선순위).
//
// 주의: OpenAI Structured Outputs(strict 모드)는 minLength/maxLength/minItems/maxItems 같은
// 길이 제약 키워드를 지원하지 않고, 스키마에 포함되면 요청 자체가 400 오류로 거부된다
// (Anthropic 구조화 출력은 이런 제약을 허용했던 부분이라 전환 과정에서 드러난 차이).
// 그래서 여기서는 .min()/.max()/.length() 같은 길이 제약을 스키마에 걸지 않고, 대신
// [1] 프롬프트(.describe() 및 시스템 프롬프트 본문)로 길이/개수 요구사항을 지시하고
// [2] 클라이언트(src/lib/api.ts)에서 응답을 받은 뒤 코드로 한 번 더 검증한다.
...
export const ReplyItemSchema = z.object({
  ...
  text: z
    .string()
    .describe(
      "실제로 그대로 보낼 수 있는 완성된 답변(빈 문자열 금지). 대화에서 감지한 언어와 동일한 언어로 작성한다.",
    ),
  translationKo: z
    .string()
    .nullable()
...
const ReplyOkSchema = z.object({
  ...
  replies: z
    ...
    .describe(
      "상황에 맞게 역할이 달라지는 답변을 정확히 4개 담는다(4개보다 많거나 적으면 안 된다). " +
        "배열 순서가 곧 추천 우선순위이며 index 0이 가장 추천하는 답변이다. 단어만 바꾼 " +
        "사실상 같은 답변 4개는 금지한다.",
    ),
  quickReactions: z
    .array(z.string())
...
const ReplyUnreadableSchema = z.object({
  ...
  message: z
    .string()
    .describe(
      "사용자에게 그대로 보여줄 짧고 정중한 안내 문구(빈 문자열 금지). 예: '대화 내용을 정확히 읽기 어렵습니다. 더 선명한 스크린샷을 올려주세요.'",
    ),
});
...
const ReplyNeedsSpeakerCheckSchema = z.object({
  ...
  lastMessagePreview: z
    .string()
    .describe("화자가 불확실한 마지막 메시지를 짧게 그대로 인용한 것(80자 이내, 빈 문자열 금지)."),
});
...
export const RefineResponseSchema = z.object({
  text: z
    .string()
    .describe("조정 요청을 반영해 다시 작성한 답변 문장(빈 문자열 금지). 원래 답변과 같은 언어로 작성한다."),
  translationKo: z
    .string()
    .nullable()
...
export const StyleProfileSchema = z.object({
  speechLevel: z.string().describe("존댓말/반말 등 격식 수준을 짧게 (예: '반말', '존댓말')"),
  averageLength: z.string().describe("평소 메시지 길이 경향을 짧게 (예: '짧은 편', '보통')"),
  emojiUsage: z.string().describe("이모티콘/이모지 사용 정도를 짧게"),
  laughterStyle: z.string().describe("ㅋㅋ/ㅎㅎ 등 웃음 표현 사용 정도를 짧게"),
  directness: z.string().describe("직접적/간접적 표현 성향을 짧게"),
});
...
export const ConversationContextSchema = z.object({
  ...
  recentContext: z.string().describe("가장 최근 상황을 한두 문장으로."),
});
